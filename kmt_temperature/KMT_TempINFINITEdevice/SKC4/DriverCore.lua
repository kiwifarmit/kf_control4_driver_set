...
function ShowProperty(propertyName)
  SetPropertyVisility(propertyName, true)
end
function HideProperty(propertyName)
  SetPropertyVisility(propertyName, false)
end

function SetPropertyVisility(propertyName, isVisible)
  if (Properties[propertyName] ~= nil) then
    if (isVisible) then
...
function UIRequest(sRequest, tParams)
  if (sRequest ~= nil) then
    ...
    if (SKC4_LOGGER ~= nil and type(SKC4_LOGGER) == "table") then
      SKC4_LOGGER:info("ReceivedFromProxy(", sRequest, ") with params ",tParams)
    end

    -- Remove any spaces (trim the property)
    local trimmedProperty = string.gsub(sRequest, " ", "_")
    local status = true
    local ret = ""
  
    if (SKC4_UI_REQUESTS[sRequest] ~= nil and type(SKC4_UI_REQUESTS[sRequest]) == "function") then
      status, ret = pcall(SKC4_UI_REQUESTS[sRequest], tParams)
    elseif (SKC4_UI_REQUESTS[trimmedProperty] ~= nil and type(SKC4_UI_REQUESTS[trimmedProperty]) == "function") then
      status, ret = pcall(SKC4_UI_REQUESTS[trimmedProperty], tParams)
    end

    if (UI_REQUESTS[sRequest] ~= nil and type(UI_REQUESTS[sRequest]) == "function") then
      status, ret = pcall(UI_REQUESTS[sRequest], tParams)
    elseif (UI_REQUESTS[trimmedProperty] ~= nil and type(UI_REQUESTS[trimmedProperty]) == "function") then
      status, ret = pcall(UI_REQUESTS[trimmedProperty], tParams)
    end
  
    if (not status) then
      if (SKC4_LOGGER ~= nil and type(SKC4_LOGGER) == "table") then
        SKC4_LOGGER:error("LUA_ERROR: ", ret)
      end
    end
